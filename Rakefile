...
task :init do
  ...
  ENV['CN'] = "Capsci Health Consul Certificate Authority #{Time.now.strftime("%s")}"
  sh "openssl req -nodes -config openssl.cnf -days 3650 -x509 -newkey rsa:2048 -out ca_crt.pem -outform PEM"
end
